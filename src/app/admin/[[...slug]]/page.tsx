...
function CategoriesManager() {
  ...
  const addCategory = async () => {
    ...
    await addCategoryAction(newName.trim(), slug);
    setNewName("");
    fetchCategories();
  };
...
const AdminDashboard = () => {
  ...
  const isPostsActive = pathname === "/admin" || pathname === "/admin/posts";
      ...
      <div className="flex">
        ...
        <aside className="w-56 border-r border-border min-h-[calc(100vh-57px)] p-6">
          <nav className="space-y-1">
            <Link href="/admin" className={`flex items-center gap-3 px-3 py-2 text-display text-sm tracking-wider transition-colors ${isPostsActive ? "text-accent" : "hover-accent"}`}>
              <FileText className="w-4 h-4" /> Posts
            </Link>
            <Link href="/admin/categories" className={`flex items-center gap-3 px-3 py-2 text-display text-sm tracking-wider transition-colors ${isCategoriesActive ? "text-accent" : "hover-accent"}`}>
        ...
        {/* Content */}
        <div className="flex-1 p-8">
          {slug === "categories" ? <CategoriesManager /> : <PostsList />}
        </div>
      </div>
    </main>
  );
