...
export function SearchButton({ isSidebarOpen }: { isSidebarOpen: boolean }) {
  ...
  return (
    <>
      <button
        ...
        className={`fixed top-6 right-6 z-50 flex items-center gap-2 px-4 py-2 border text-sm text-display tracking-widest transition-all duration-300 ${isSidebarOpen
          ? "border-black/20 text-black hover:border-accent hover:text-accent"
          : "border-transparent text-foreground hover:bg-white/10 dark:hover:bg-white/10 hover:border-white/20"
          }`}
      >
